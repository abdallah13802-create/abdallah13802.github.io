...
<!-- ✏️ FIX #4: Removed "Python" from tech stack since no Python script is shown in this project. Replaced with "Field Calculator (QGIS)" which was actually used for the composite scoring formula. -->
...
### 💼 Business Context
As founder of Orange AdTech, I serve 4 local service business clients operating in Orange County's highest-opportunity zones. Each client is located in a premium income corridor — Corona del Mar ($198k), Newport Beach ($168k–$185k), Coto de Caza ($178k), and Yorba Linda ($118k–$128k).
 
<!-- ✏️ FIX #1: Changed "30-minute drive-time service radius" to "15-mile service radius buffer" throughout. QGIS buffer analysis is distance-based, not true network drive-time routing. Using consistent and technically accurate language. -->
The buffer circles on the map represent each client's 15-mile service radius — the realistic zone within which their customers will travel. Buffer radius is used as a proxy for approximate service area. This constraint is critical: there is no point running Meta ads targeting zip codes far outside the client's service footprint if customers won't make the trip.
 
<!-- ✏️ FIX #2: Removed the claim that "all 4 client locations sit inside Orange County's highest-scoring opportunity zones" — Yorba Linda scores in the REVIEW tier, not INCLUDE, which contradicts that statement. -->
...
- **Client Validation:** 3 of 4 client locations independently score in the top INCLUDE tier; Yorba Linda scores in the REVIEW tier — all above the EXCLUDE threshold, confirming sound business positioning
